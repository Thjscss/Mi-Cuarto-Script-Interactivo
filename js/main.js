...
// --- Programa principal ---
function verificarStock(inventario, productoBuscado) {
    for (const producto of inventario) {
        if (producto.trim().toLowerCase() === productoBuscado.toLowerCase()) {
            return true;
        }
    }
    return false;
...
const sumarAlTotal = (montoActual, monto) => montoActual + monto;
 
// Reemplazo del objeto "cafes": dos arrays paralelos (clave <-> nombre)
const opcionesCafe = ["1", "2", "3"];
const nombresCafe = ["cafe de watemala", "cafe colombiano", "cafe peruano"];
 
function buscarNombreCafe(opcion) {
    const indice = opcionesCafe.indexOf(opcion);
    if (indice === -1) return null;
    return nombresCafe[indice];
}
 
// Función declarada: en vez de devolver un objeto {nombre, precio}, devuelve un array [nombre, precio]
function pedidoBebida(opcionesBebida) {
    ...
    return [bebidaElegida, precioBebida];
}
 
// Función expresada: también devuelve un array [nombre, precio]
const pedidoComida = function (opcionesComida) {
    ...
    return [comidaElegida, precioComida];
};
...
while (seguirBebida) {
    ...
    if (!["1", "2", "3", "4", "5"].includes(menuBebida)) {
        ...
        const nombreCafeElegido = buscarNombreCafe(tipoCafe);
 
        if (!nombreCafeElegido || !verificarStock(inventarioBebida, nombreCafeElegido)) {
            alert("No contamos con stock. Por favor, elija otra opción del 1 al 4.");
            continue;
        }
 
        const resultado = pedidoBebida(menuBebida);
        bebidaFinal = nombreCafeElegido;
        totalPrecio = sumarAlTotal(totalPrecio, resultado[1]);
    } else {
        const resultado = pedidoBebida(menuBebida);
        bebidaFinal = resultado[0];
        totalPrecio = sumarAlTotal(totalPrecio, resultado[1]);
    }
    seguirBebida = false;
}
...
while (seguirComida) {
    ...
    if (resultado[0]) {
        comidaFinal = resultado[0];
        totalPrecio = sumarAlTotal(totalPrecio, resultado[1]);
    }
    seguirComida = false;
}
 
// En vez de devolver un objeto {bebida, comida, total}, devuelve un array [bebida, comida, total]
function mostrarTicket(bebidaElegida, comidaElegida, totalPrecio) {
    ...
    return [bebidaElegida, comidaElegida, totalPrecio];
}
